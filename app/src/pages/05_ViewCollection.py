...
try:
    ...
    cID = st.session_state['select_collect']
    rURL = 'http://web-api:4000/col/collections/' + str(cID)
...
st.title(userJSON[0]["title"])

st.write(userJSON[0]["description"])

try:
    rURL = 'http://web-api:4000/col/collections/' + str(cID) + '/recipes'
    alt_response = requests.get(rURL)
    altJSON = alt_response.json() if alt_response.status_code == 200 else []
except:
    altJSON = []
    st.write('# ERROR! CANNOT GET USER DATA!')
